...
def set_comp_device(FLAG_GPU):
    ...
    if FLAG_GPU:
        device_comp = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")
    return device_comp
...
def set_model_name(OUTPUT_PARAMS, DATA_PARAMS, TRAIN_PARAMS):
    model_name = 'A' + str(TRAIN_PARAMS['ARCH_NUM']).zfill(2) + '_D' + str(DATA_PARAMS['DATA_SET']).zfill(2) + '_T' + \
                 str(OUTPUT_PARAMS['EXP_TRY']).zfill(2) + '_on_Adience'
    return model_name
...
def load_model(model_dir, model_name, arch_name, device_comp, arch_type, DATA_PARAMS, epoch_start = 0, ch_inp_num = 6, label_num=1200):
    ...
    if epoch_start > 0:
        if arch_name=='Generator':
            model.load_state_dict(torch.load(model_dir + model_name + '_ep' + str(epoch_start).zfill(5) + 'G.pth'))
            print("Model loaded:", model_name, " epoch:", str(epoch_start))
        elif arch_name=='Discriminator':
            model.load_state_dict(torch.load(model_dir + model_name + '_ep' + str(epoch_start).zfill(5) + 'C.pth'))
            print("Model loaded:", model_name, " epoch:", str(epoch_start))
        elif arch_name=='ResNet' or arch_name=='NLayerDiscriminator':    
            model.load_state_dict(torch.load(model_dir + model_name + '_ep' + str(epoch_start).zfill(5) + 'S.pth'))
            print("Model loaded:", model_name, " epoch:", str(epoch_start))
