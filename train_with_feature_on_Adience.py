...
class Train_GAN():
    def __init__(self, model_info, device_comp, margin_contrastive=3, num_classes = 1200, gan_type = 'lsgan'):
        ...
        self.L1_loss = util_loss.L1_plus_perceptualLoss(self.device_comp).to(self.device_comp)
        self.criterionSSIM = util_loss.SSIM(win_size=11, win_sigma=1.5, data_range=1.0,
                                          size_average=True)
...
#         self.parser.add_argument('--win_sigma', type=float, default=1.5, help='the window size of SSIM conputation')
    ...
    def onehot_labelmap(self, img_mask):
#         print(img_mask.size())
        label_map = img_mask.unsqueeze(1)
#         print(label_map.size())
    ...
    def process_batch_data(self, input_batch, flag_same = False):
        im_faces, im_lndm, im_msk, im_ind, im_features = input_batch

        im_faces = [item.float().to(self.device_comp) for item in im_faces]
        im_lndm = [self.onehot_labelmap(item).float().to(self.device_comp) for item in im_lndm]
        im_msk = [item.float().to(self.device_comp) for item in im_msk]
        im_features = [item.float().to(self.device_comp) for item in im_features]
        ...
        if flag_same:
            ...
            return im_faces, im_lndm, im_msk, labels_one_hot, label_same.to(self.device_comp), im_features

        return im_faces, im_lndm, im_msk, labels_one_hot, im_features
    ...
    def train_siamese(self, num_iter_siamese=1):
        ...
        for j in range(num_iter_siamese):
            ...
            im_faces, im_lndm, im_msk, im_onehot, label_data, _ = self.process_batch_data(sample_batch, flag_same = True)
    ...
    def input_train(self, im_faces, im_lndm, im_msk):
        input_repr = im_lndm
#         print(im_faces[0].size(), im_lndm.size(),  im_msk[0].size())
        bg = im_faces[0] * (1 - im_msk[0])
        input_gen = torch.cat((input_repr, im_faces[0] * (1 - im_msk[0])), 1)
        return input_gen, input_repr, im_msk, bg
    ...
    def train_critic(self, num_iter_critic=1):
        ...
        for j in range(num_iter_critic):
            ...
            with torch.no_grad():
                sample_batch = self.data_iter.next()
                im_faces, im_lndm, im_msk, im_onehot, im_features = self.process_batch_data(sample_batch)

                input_gen, input_repr, im_msk, bg = self.input_train(im_faces, im_lndm[0], im_msk)
                 
                im_gen = self.model_info['generator'](input_repr, bg, im_features[1])
            ...
            _, input_repr, im_msk, _ = self.input_train(im_faces, im_lndm[0], im_msk)
    ...
    def train_generator(self, num_iter_generator=1, flag_siamese = True):
        ...
        for j in range(num_iter_generator):
            ...
            sample_batch = self.data_iter.next()
            im_faces, im_lndm, im_msk, im_onehot, label_same, im_features = self.process_batch_data(sample_batch, flag_same = True)
            ...
            loss_L1,_,_ = self.L1_loss(output_gen, im_faces[0])
            loss_ssim = (1 - self.criterionSSIM(output_gen, im_faces[0]))
...
#             loss_G = loss_G_gan + loss_G_siam + loss_L1 + loss_ssim

            loss_G = loss_G_gan/loss_G_gan.detach() +  loss_G_siam/loss_G_siam.detach() +   loss_L1/loss_L1.detach()  +   loss_ssim/loss_ssim.detach()
            # print(loss_G_gan, loss_G_siam, loss_L1, loss_ssim)
            loss_G.backward()
...
def run_exp(TRAIN_PARAMS):
    ...
    org_model_dir = '/home/junjie/DeIDVideo/SemanticImageSynthesis/ciagan_semantic/models/ciagan_Aspade_feature_flex_DCeleBA_Tcheckhyperparameters_att__unitloss_sknoise/'
    org_model_name = 'Aspade_feature_flex_DCeleBA_Tcheck'

    ##### PREPARING MODELS
    ch_inp_num = 11
    generator = load_model(org_model_dir, org_model_name, 'Generator', device_comp, TRAIN_PARAMS['ARCH_NUM'], epoch_start = TRAIN_PARAMS['EPOCH_START'], ch_inp_num = 11, label_num=label_num)
    critic = load_model(org_model_dir, org_model_name, 'Discriminator', device_comp, TRAIN_PARAMS['ARCH_NUM'], epoch_start = TRAIN_PARAMS['EPOCH_START'], ch_inp_num = 14)

    if TRAIN_PARAMS['ARCH_SIAM'][:6]=='resnet':
        siamese = load_model(org_model_dir, org_model_name, 'ResNet', device_comp, TRAIN_PARAMS['ARCH_SIAM'], epoch_start=TRAIN_PARAMS['EPOCH_START'], ch_inp_num = 3)
    elif TRAIN_PARAMS['ARCH_SIAM'][:4] == 'siam':
        siamese = load_model(org_model_dir, org_model_name, 'NLayerDiscriminator', device_comp, TRAIN_PARAMS['ARCH_SIAM'], epoch_start=TRAIN_PARAMS['EPOCH_START'], ch_inp_num = 3)
    ...
    model_info = {'generator': generator,
                  ...
                  'label_num': label_num,
                  'LAMBDA1':TRAIN_PARAMS['LAMBDA1'],
                  'LAMBDA2':TRAIN_PARAMS['LAMBDA2'],
                  'LAMBDA3':TRAIN_PARAMS['LAMBDA3'],
                  }
