...
#         print(img_res.shape)
    ...
    def __getitem__(self, idx):
        ...
        for k_iter in range(self.flag_sample):
            ...
            im_semantic_path = im_clr_path.replace('.jpg', '_semantic.npy')
            im_feature_path = im_clr_path.replace('.jpg', '_feature_refined.npy')

            semantic_img = torch.from_numpy(  np.load( im_semantic_path )  ) 
            hidden_feature = torch.from_numpy( np.load( im_feature_path ) )

            im_feature.append(hidden_feature)
             
#             lndm_img = self.onehot_labelmap(semantic_img)
        ...
        return im_clr, im_lndm, im_msk, im_ind, im_feature
